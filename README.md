...
This compiles the client into a jar located at `./target/AwsApiGateway-1.0-SNAPSHOT.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'io.apigateway:AwsApiGateway:1.0-SNAPSHOT'
}
```
...
<dependencies>
    <dependency>
        <groupId>io.apigateway</groupId>
        <artifactId>AwsApiGateway</artifactId>
...
This will compile the client and package all dependencies into a jar located at `./target/AwsApiGateway-1.0-SNAPSHOT.jar`.
...
```java
AwsApiGatewayClientBuilder builder = AwsApiGateway.builder();
```
...
```java
AwsApiGateway client = AwsApiGateway.builder()
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`AwsApiGateway`) is created for all actions (resource + method) in your API.
...
```java
AwsApiGateway client = AwsApiGateway.builder().build();
GetGetpresignedurlfordownloadResult result = client.getGetpresignedurlfordownload(new GetGetpresignedurlfordownloadRequest());
```
...
```java
AwsApiGateway client = AwsApiGateway.builder().build();
client.getGetpresignedurlfordownload(new GetGetpresignedurlfordownloadRequest().sdkRequestConfig(
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of AwsApiGatewayException.
```java
try {
    client.getGetpresignedurlfordownload(...);
} catch(AwsApiGatewayException e) {
   // All service exceptions will extend from AwsApiGatewayException.
   // Any unknown or unmodeled service exceptions will be represented as a AwsApiGatewayException.
